...
fn manager_resource_path(resource: &str) -> CliResult<&'static str> {
    match resource {
        ...
        "provider-custom" | "providers/custom" => Ok("/v1/admin/providers/custom"),
        "agent" => Ok("/v1/admin/agent"),
        "runtime" => Ok("/v1/admin/runtime"),
        "context" => Ok("/v1/admin/context"),
        "sessions" => Ok("/v1/admin/sessions"),
        "runs" | "tasks" => Ok("/v1/admin/runs"),
...
fn render_human(command: &str, value: &Value) {
    let output = crate::cli_contract::render_human_for_command(command, value);
    if !output.is_empty() {
        println!("{output}");
    }
}
